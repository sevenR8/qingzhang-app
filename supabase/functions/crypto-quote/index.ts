...
const yahooChart = async (symbol: string, asOf = '') => {
  ...
  return {
    name: String(result?.meta?.shortName || result?.meta?.longName || ''),
    price,
...
Deno.serve(async (request) => {
  ...
  if (!symbols.length) return json({ error: '沒有可查詢的加密貨幣代碼' }, 400);
  if (symbols.length > 20) return json({ error: '一次最多更新 20 種加密貨幣' }, 400);
  ...
  const results = await Promise.all(symbols.map(async (symbol) => {
    try {
      const quote = await yahooChart(`${symbol}-USD`, asOf);
